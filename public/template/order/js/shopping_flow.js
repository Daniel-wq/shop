...
function selectPayment(obj)
{
 ...
 var rec =  Ajax.call('flow.php?step=select_payment', 'payment=' + obj.value, null, 'GET', 'JSON',false);

	if(rec.error != '')
	{
		$('#address').addClass('seleadd');
		alert(rec.error);
...
/* 当用户提交到订单页面时，自动帮用户选中支付宝 */
function selectPaymentValue(v)
{
  ...
  Ajax.call('flow.php?step=select_payment', 'payment=' + v, orderSelectedResponse, 'GET', 'JSON');	
}

/* 当用户提交到订单页面时，自动帮用户选中宅急送 */
function selectShippingValue(v)
{
  var s = document.getElementsByName('shipping');
  ...
  Ajax.call('flow.php?step=select_shipping', 'shipping=' + v, orderShippingSelectedResponse, 'GET', 'JSON');
}
...
function checkConsignee(frm)
{
  ...
  {
	err = true;
	msg.push('电话和手机号码必填一项');
  }
	  
  if (!Utils.isTel(frm.elements['tel'].value) && frm.elements['tel'].value.length > 0)
  {
    err = true;
  ...
  if (!/^\d{11}$/.test(frm.elements['mobile'].value) && frm.elements['mobile'].value.length > 0)
  {
    err = true;
...
function check_cart()
{
	...
	$('input[name="items"]').each(function(){
		if($(this).attr('checked')==false){
			var res = Ajax.call('flow.php?step=drop_goods', 'id='+$(this).val() ,null, 'POST', 'JSON', false);	
		};
	});
	//进入订单页
 ...
 * 改变余额
 */
function changeAcount(val)
{
  if (selectedAcount == val)
  {
    return;
  }
  else
  ...
  Ajax.call('flow.php?step=change_Acount', 'is_use_userMoney=' + val, changeAcountResponse, 'GET', 'JSON');
}

/* *
 * 改变余额回调函数
 */
function changeAcountResponse(obj)
{
  if (obj.error)
  {
    try
